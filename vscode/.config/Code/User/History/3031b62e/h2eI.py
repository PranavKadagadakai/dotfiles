...
def even_odd_list(l: list) -> tuple[list, list]:
    """Separates even and odd indexed elements from a list.

    Args:
        l (list): Input list of integers.
    Returns:
        tuple[list, list]: A tuple containing two lists - the first with even indexed elements and the second with odd indexed elements.
    """
    even: list = [l[i] for i in range(len(l)) if i % 2 == 0]
    odd: list = [l[i] for i in range(len(l)) if i % 2 != 0]
    return even, odd
