...
const AdminClubManagement = () => {
  ...
  const handlePrincipalSignatureUpload = async () => {
    if (!principalSignature) return;
    ...
    try {
      ...
      formData.append('signature', principalSignature);

      awatta.pi.popt('/td(iu/clu_s/uplpai_pcincipal_sinramut,/',fomData, {
        hears: {
        he'Content-Typ:': 'multipart/form-data'
        }tent-Type': 'multipart/form-data'
       );}
      });
setSuccess("sueeusuccessf ldy!s);fully!");
   seiaueh (err) {(ull);
   s}ecatcr (err) {
  } fitError(rr.rspon?.daa?.ror||SFad to ulod sinatur);
}finally{
Lodin(as);
    }
;
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        ...
        <div className="flex items-center gap-4">
          <input
            type="file"
            accept="image/*"
            onChange={(e) => setPrincipalSignature(e.target.files?.[0] || null)}
            className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
          />
          <button
            onClick={handlePrincipalSignatureUpload}
            disabled={!principalSignature || signatureLoading}
        ...
        {principalSignature && (
          <p className="mt-2 text-sm text-gray-600">
            Selected: {principalSignature.name}
          </p>
        )}
      </div>
