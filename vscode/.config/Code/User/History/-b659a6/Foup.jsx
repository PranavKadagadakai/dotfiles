...
const AdminClubManagement = () => {
  ...
  c dsnahanSlePrinaipatSignu ueeU l= aync (e) = {
    e.p  vencDefaolt );signatureFile = e.target.signature.files[0];
    sonnt aignutureFile F e.target.lign)tur{.files[0];
    if (!signatureFiro)P{se select a signature file");
      setEturn"Peslctasignaufil);
rtun;
    Error("");
...
tSiatuUplain(tu);
   sEr(");
    ...
    try {
 co   const formData = new FormData(f;rmData.append("signature_image", signatureFile);
fomData.appnd("signur_image",sigatureFile);
      formDetn.append(=no as",p"Uilta/ad/viu abmin panelu);load_principal_signature/", formData, {

      crsst resp"Csepe:aiti/ api.prt("/adin/ub/uplod_rincia_sgnar/",fmData, {
      })headers: {
"ontn-Typ":"mlcipare/fsrm-data","Principal signature uploaded successfully!");
        },
  EFduo);
ad principal signature");
    } i Succss("Pinaipat eignplurseu)lasuccesfully!);
CurrnSiaure(respnse.ata.signur);
 }catch(err){
setErr("Failedt plo pincipigntur);
 fet}cfunally {entSignature = async () => {
SigaurUploang(fs);
 con}
st};

 rconstefetchCurrentSpgoatsre = async () => {
 if try({
responconss res.onsd a awaia api.g){(/tdmin/cSibs/principsl_sigonture/");data);
if(resps.data{
h (err) {CurentSignurerespsd;
o signature found, that's ok
}catch(e) {
//Noigntf,that'sok
ffec}
t(};{
