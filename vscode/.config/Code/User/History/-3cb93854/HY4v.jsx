...
const AdminMenteeAssignment = () => {
  ...
  const fetchMenteeAssignments = async () => {
    try {
      setLoading(true);
      const response = await api.get("/admin/mentees/list/");
  ...
  const handleBulkUpload = async (e) => {
    ...
    try {
      ...
      setSuccess(`Assigned ${response.data.assigned} mentees. Errors: ${response.data.errors.length}`);
      setCsvFile(null);
      fetchMenteeAssignments();
    } catch (err) {
  ...
  if (loading) return <div className="text-center py-10">Loading mentee assignments...</div>;
        ...
        <form onSubmit={handleBulkUpload} className="space-y-4">
          <div>
            <label className="block text-sm font-medium mb-2">
              Upload CSV (Format: mentor_id, student_usn)
            </label>
            <input
              type="file"
              accept=".csv"
            ...
            {mentorMenteeData.map((mentor) => (
              <div key={mentor.mentor_id} className="border rounded-lg p-4">
                ...
                <div className="text-sm text-gray-600 mb-3">
                  Total Mentees: <span className="font-bold">{mentor.mentee_count}</span>
                </div>
