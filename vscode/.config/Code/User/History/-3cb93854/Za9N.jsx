...
const AdminMenteeAssignment = () => {
  ...
  const handleFileChange = async (e) => {
    ...
    const validation = await validateBulkUploadFile(file, "mentee_assignment");

    if (!validation.isValid) {
      setError(`File validation failed: ${validation.error}`);
      setCsvFile(null);
      setCsvValidation(null);
          ...
          <div>
            ...
            {csvFile && csvValidation && (
              <div className="mt-2 text-xs text-green-600">
                ✓ File validated: {formatFileSize(csvFile.size)} •{" "}
                {csvValidation.preview?.dataRows || "Unknown"} rows
              </div>
            )}
          </div>
          <button
