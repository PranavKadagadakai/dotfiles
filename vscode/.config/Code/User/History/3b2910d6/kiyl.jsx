...
const HallBookingForm = ({ onBookingCreated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.hall || !formData.booking_date || !formData.start_time || !formData.end_time) {
    ...
    try {
      ...
      setError(
        err.response?.data?.detail || "Failed to create booking. Please check for conflicts."
      );
    } finally {
      setLoading(false);
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 py-2">From {formData.start_time || "--:--"} to {formData.end_time || "--:--"}</p>
        </div>
      </div>
