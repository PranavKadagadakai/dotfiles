...
class MentorProfileSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        # Handle file fields: check both initial_data and FILES for uploaded files
        request = self.context.get('request')
        file_fields = ["profile_photo", "signature"]

        for field_name in file_fields:
            # Check if file was uploaded in request.FILES
            if request and request.FILES.get(field_name):
                uploaded_file = request.FILES[field_name]
                setattr(instance, field_name, uploaded_file)
                # Remove from validated_data if present to prevent double processing
                if field_name in validated_data:
                    del validated_data[field_name]
            # If field is in validated_data and is None, it means we want to clear it
