...
class MentorProfileSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        # Handle file fields: only update if explicitly provided in the request
        file_fields = ["profile_photo", "signature"]
        for field_name in file_fields:
            if field_name in self.initial_data:  # Check if field was in original request
                if field_name in validated_data:
                    # Only update if it's a file object or explicitly None (to clear)
                    if hasattr(validated_data[field_name], 'read') or validated_data[field_name] is None:
                        setattr(instance, field_name, validated_data[field_name])
                    # Remove from validated_data to prevent double processing
