...
class MentorProfileSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        # Handle file fields: only update if a new file is provided or explicitly set to None (to clear)
        file_fields = ["profile_photo", "signature"]
        for field_name in file_fields:
            if field_name in validated_data:
                # Only update if it's a file object (has read method) or explicitly None (to clear)
