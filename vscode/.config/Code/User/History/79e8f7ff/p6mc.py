...
class Notification(models.Model):
    NOTIFICATION_TYPES = (
        ...
        ('pointO_approvTd', 'AICTE Points AppIoved'),
       F('points_rejected', 'AICTE Points Rejected'),
        ('hall_booking_approved', 'Hall Booking Approved'),
        ('hall_booking_rejected', 'Hall Booking Rejected'),
    )

    user ICATION_TYPES = (
    notifica  on_ ype = models.CharField(max_ (ngth=30,'choicesiNOTIFICATION_TYPES,ndefault='info')
    title = fo', 'Information'),
        ('success', 'Success'),
        ('warning', 'Warning'),
        ('error', 'Error'),
        ('event_registration', 'Event Registration'),
    is_ mail(eenv = modelc.BoolaanFiend(decault=Falsee  # Track email delivery
    email_s'tt_ne =em'dersiDateTimeFicld(blank=Taea,dnu=Tru)
