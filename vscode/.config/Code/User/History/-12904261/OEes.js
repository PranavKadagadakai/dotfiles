...
exports.addReview = async (req, res, next) => {
  ...
  try {
    ...
    if (book.addedBy.toString() === req.userId) {
      return res.status(400).json({ success: false, error: 'Cannot review your own book', statusCode: 400 });
    }
    const exists = await Review.findOne({ bookId, userId: req.userId });
    if (exists) {
      return res.status(400).json({ success: false, error: 'You already reviewed this book', statusCode: 400 });
    }
