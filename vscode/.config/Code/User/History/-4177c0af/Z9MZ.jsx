        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            {reportData.total_registered !== undefined && (
              <div className="bg-green-50 p-4 rounded-lg border-l-4 border-green-500">
                ...
                <p className="text-3xl font-bold text-green-600">{reportData.total_registered}</p>
              </div>
            )}
            {reportData.total_attended !== undefined && (
              <div className="bg-purple-50 p-4 rounded-lg border-l-4 border-purple-500">
                <p className="text-sm text-gray-600">Total Attended</p>
                <p className="text-3xl font-bold text-purple-600">{reportData.total_attended}</p>
              </div>
            )}
            {reportData.total_points !== undefined && (
              <div className="bg-orange-50 p-4 rounded-lg border-l-4 border-orange-500">
                <p className="text-sm text-gray-600">AICTE Points Earned</p>
                <p className="text-3xl font-bold text-orange-600">{reportData.total_points}</p>
              </div>
            )}
          </div>
          ...
          {reportData.event_breakdown && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.entries(reportData.event_breakdown).map(([status, count]) => (
          ...
          {reportData.pending_approvals !== undefined && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              ...
              <p className="text-2xl font-bold text-yellow-700">{reportData.pending_approvals}</p>
              <p className="text-xs text-yellow-600 mt-2">Items awaiting your action</p>
            </div>
          )}
        </div>
      )}
