...
from .email_utils import (
    ...
    send_hall_booking_decision_email,
    send_password_reset_email,
    send_points_decision_email,
...
from .permissions import IsAdmin, IsClubAdmin, IsMentor, IsStudent
from .serializers import (
    AICTECategorySerializer,
    AICTEPointTransactionSerializer,
    AuditLogSerializer,
    CertificateSerializer,
    CertificateTemplateSerializer,
    ClubMemberSerializer,
    ClubOrganizerProfileSerializer,
    ClubOrganizerSerializer,
    ClubRoleSerializer,
    ClubSerializer,
...
class ProfileView(generics.RetrieveUpdateAPIView):
    ...
    def get_serializer_class(self):
        ...
        if user.user_type == "student" and Student.objects.filter(user=user).exists():
            return StudentProfileSerializer
        elif user.user_type == "mentor" and Mentor.objects.filter(user=user).exists():
            return MentorProfileSerializer
        elif user.user_type == "club_organizer" and ClubOrganizer.objects.filter(user=user).exists():
    ...
    def get_object(self):
        ...
        if user.user_type == "student" and Student.objects.filter(user=user).exists():
            ...
            return ClubOrganizer.objects.get(user=user)
        return user
    ...
    def perform_update(self, serializer):
        ...
        # Handle file uploads manually before serializer.save()
        if hasattr(instance, 'profile_photo') and 'profile_photo' in self.request.FILES:
            instance.profile_photo = self.request.FILES['profile_photo']
        if hasattr(instance, 'signature') and 'signature' in self.request.FILES:
            instance.signature = self.request.FILES['signature']
        ...
        log_action(
            self.request.user, f"Updated profile ({self.request.user.user_type})"
        )
...
class StudentProfileView(generics.RetrieveUpdateAPIView):
    ...
    def perform_update(self, serializer):
        ...
        if (
            all(getattr(student, field, None) for field in required_fields)
...
class MentorProfileView(generics.RetrieveUpdateAPIView):
    ...
    def perform_update(self, serializer):
        ...
        if (
            all(getattr(mentor, field, None) for field in required_fields)
            and not mentor.profile_completed
        ):
...
class ClubOrganizerProfileView(generics.RetrieveUpdateAPIView):
    ...
    def perform_update(self, serializer):
        ...
        if (
            all(getattr(organizer, field, None) for field in required_fields)
            and not organizer.profile_completed
        ):
...
class StudentViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        participated_events = (
            EventAttendance.objects.filter(student=instance, is_present=True)
            .select_related("event")
            .values("event", "event__name")
        )
        ...
        registered_events = (
            EventRegistration.objects.filter(student=instance)
            .select_related("event")
            .values("event", "event__name")
        )
        ...
        # Certificates earned
        certificates = Certificate.objects.filter(student=instance).select_related(
            "event"
        )
        ...
        pending_data = [
            {
                ...
                if certificates.filter(event=tx.event).exists()
                else None,
            }
        ...
        data["detailed_stats"] = {
            "aicte_points": {
                ...
                "progress_percentage": (total_aicte_points / required_points * 100)
            ...
            "events": {
                ...
                "registered": [
                    {"event_id": item["event"], "event_name": item["event__name"]}
                    for item in registered_events
                ],
                "participated": [
                    {"event_id": item["event"], "event_name": item["event__name"]}
                    for item in participated_events
                ],
            },
            "certificates": {"count": certificates_count, "list": certificates_data},
            "approvals": {
...
class ClubOrganizerViewSet(viewsets.ModelViewSet):
    ...
    def assign_club(self, request, pk=None):
        ...
        if not club_id:
            return Response(
                {"error": "club_id is required"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        organizer.save()
        log_action(
            request.user, f"Assigned {organizer.user.username} to club {club.name}"
        )
        return Response(
            {"message": f"{organizer.user.get_full_name()} assigned to {club.name}"},
            status=status.HTTP_200_OK,
        )
                ...
                try:
                    ...
                    if user_type == "student":
                        ...
                        if not all([employee_id, designation]):
                            errors.append(
                                f"Row {row_num}: Employee ID and Designation required for mentor"
                            )
                            user.delete()
                            continue
                    ...
                    log_action(
                        request.user, f"Created user via bulk import: {username}"
                    )
...
class MentorMenteeAssignmentView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        if not mentor_id:
            return Response(
                {"error": "Mentor ID is required."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        for student_id in student_ids:
            try:
                ...
                log_action(
                    request.user,
                    f"Assigned student {student.usn} to mentor {mentor.employee_id}",
                )
...
class BulkMenteeAssignmentView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            for row_num, row in enumerate(csv_reader, start=2):
                try:
                    ...
                    if not all([mentor_emp_id, student_usn]):
                        errors.append(
                            f"Row {row_num}: Missing mentor_employee_id or student_usn"
                        )
                        continue
                    ...
                    if not mentor:
                        errors.append(
                            f"Row {row_num}: Mentor with ID {mentor_emp_id} not found"
                        )
                        continue

                    if not student:
                        errors.append(
                            f"Row {row_num}: Student with USN {student_usn} not found"
                        )
                        continue
                    ...
                    log_action(
                        request.user,
                        f"Bulk assigned student {student_usn} to mentor {mentor_emp_id}",
                    )
            ...
            return Response(
                {"assigned_count": assigned_count, "errors": errors},
                status=status.HTTP_200_OK,
            )
...
class AdminUserListViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def disable_account(self, request, pk=None):
        ...
        return Response(
            {"message": f"User {user.username} account disabled."},
            status=status.HTTP_200_OK,
        )
    ...
    def enable_account(self, request, pk=None):
        ...
        return Response(
            {"message": f"User {user.username} account enabled."},
            status=status.HTTP_200_OK,
        )
    ...
    def unlock_account(self, request, pk=None):
        ...
        return Response(
            {"message": f"User {user.username} account unlocked."},
            status=status.HTTP_200_OK,
        )
...
class AdminClubManagementViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = Club.objects.prefetch_related(
            "faculty_coordinator", "club_head", "members"
        )
        if search:
            queryset = queryset.filter(
    ...
    def perform_create(self, serializer):
        ...
        log_action(
            self.request.user,
            f"Created club: {club.name} with coordinator: {mentor.user.get_full_name()}",
        )
    ...
    def assign_organizer(self, request, pk=None):
        ...
        try:
            ...
            log_action(
                request.user,
                f"Assigned {organizer_profile.user.username} to club {club.name}",
            )
            return Response(
                {
            ...
            return Response(
                {"error": "Club organizer not found"}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def assign_coordinator(self, request, pk=None):
        ...
        try:
            ...
            log_action(
                request.user,
                f"Changed coordinator of {club.name} to {mentor.user.get_full_name()}",
            )
            return Response(
                {
    ...
    def assign_club_head(self, request, pk=None):
        ...
        try:
            ...
            log_action(
                request.user,
                f"Assigned {student.user.username} as club head to club {club.name}",
            )
            return Response(
                {"message": f"{student.user.get_full_name()} assigned as club head."},
                status=status.HTTP_200_OK,
            )
        except Student.DoesNotExist:
            return Response(
    ...
    def principal_signature(self, request):
        ...
        if signature:
            ...
            return Response(serializer.data)
        return Response(
            {"detail": "No active principal signature found"},
            status=status.HTTP_404_NOT_FOUND,
        )
    ...
    def upload_principal_signature(self, request):
        ...
        log_action(
            request.user, f"Uploaded new principal signature: {principal_signature}"
        )
            ...
            data.append(
                {
                    ...
                    "mentees": [
                        {"id": s.id, "usn": s.usn, "name": s.user.get_full_name()}
                        for s in mentor.mentees.all()
                    ],
                }
            )
        ...
        try:
            ...
            for row_num, row in enumerate(csv_reader, start=2):
                try:
                    ...
                    if not mentor:
                        errors.append(
                            f"Row {row_num}: Mentor with employee ID {mentor_emp_id} not found"
                        )
                        continue
                    ...
                    log_action(
                        request.user,
                        f"Assigned mentee {student_usn} to mentor {mentor_emp_id}",
                    )
                except Exception as e:
                    errors.append(f"Row {row_num}: {str(e)}")

            return Response(
                {"assigned": assigned, "errors": errors}, status=status.HTTP_200_OK
            )
        except Exception as e:
...
class AdminReportingViewSet(viewsets.ViewSet):
    ...
    def aicte_compliance_report(self, request):
        ...
        regular_completed = regular_students.filter(total_aicte_points__gte=100).count()
        regular_pending = regular_students.count() - regular_completed

        lateral_completed = lateral_students.filter(total_aicte_points__gte=75).count()
        lateral_pending = lateral_students.count() - lateral_completed

        # Overall statistics
        total_points_awarded = (
            AICTEPointTransaction.objects.filter(status="APPROVED").aggregate(
                total=Sum("points_allocated")
            )["total"]
            or 0
        )
        ...
        report = {
            ...
            "department_wise": list(
                ...
                .annotate(
                    ...
                    regular_students=Count(Q(admission_type="regular")),
                    lateral_students=Count(Q(admission_type="lateral")),
    ...
    def audit_logs(self, request):
        ...
        logs = AuditLog.objects.filter(timestamp__gte=cutoff_date).order_by(
            "-timestamp"
        )
    ...
    def hall_utilization_report(self, request):
        ...
        halls = Hall.objects.annotate(
            ...
            approved_bookings=Count(
                "bookings", filter=Q(bookings__booking_status="APPROVED")
            ),
            pending_bookings=Count(
                "bookings", filter=Q(bookings__booking_status="PENDING")
            ),
        ).values(
            "id",
            "name",
...
class ClubMemberViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        log_action(
            self.request.user,
            f"Added {club_member.student.user.username} to club {club_member.club.name}",
        )
...
class EventViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        log_action(
            user,
            f"Created Event: {event.name} (AICTE: {getattr(event.aicte_category, 'name', 'None')}, Points: {event.points_awarded})",
        )
    ...
    def perform_update(self, serializer):
        ...
        if (
            ...
            and hasattr(event, "_original_status")
            and event._original_status != "scheduled"
        ):
        ...
        if (
            ...
            and hasattr(event, "_original_status")
            and event._original_status != "cancelled"
        ):
            self._send_cancellation_emails(
                event, reason=request.data.get("cancellation_reason", "")
            )
    ...
    def _assign_hall_to_event(self, event):
        ...
        if hall_assigned:
            ...
            try:
                ...
                log_action(
                    event.created_by,
                    f"Auto-assigned hall {event.assigned_hall.name} to event {event.name}",
                )
            except Exception as e:
            ...
            log_action(
                event.created_by,
                f"Failed to assign hall to event {event.name} - no availability",
            )
    ...
    def _send_cancellation_emails(self, event, reason=""):
        ...
        registrations = EventRegistration.objects.filter(
            event=event, status__in=["REGISTERED", "ATTENDED"]
        ...
        for registration in registrations:
            ...
            try:
                threading.Thread(
                    target=send_event_cancellation_email,
                    args=(student.user, event, reason),
                ).start()
            except Exception as e:
    ...
    def register(self, request, pk=None):
        ...
        try:
            certificate_url = request.build_absolute_uri(
                "/certificates"
            )  # Placeholder for certificate download URL
            threading.Thread(
                target=send_event_registration_email,
                args=(student.user, event, certificate_url),
            ).start()
        except Exception as e:
    ...
    def upload_attendance(self, request, pk=None):
        ...
        if user.user_type != "club_organizer":
            raise PermissionDenied(
                "Only club organizers can upload attendance for events."
            )
        ...
        if not isinstance(attendance_items, list):
            return Response(
                {"error": "Attendance payload must be a list"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        with transaction.atomic():
            for item in attendance_items:
                ...
                student_id = (
                    item.get("student_id") or item.get("studentId") or item.get("id")
                )
                usn = item.get("usn") or item.get("USN")
                attendance_flag = (
                    item.get("attendance")
                    or item.get("status")
                    or item.get("is_present")
                )
                ...
                if is_present and event.aicte_category and event.points_awarded > 0:
                    ...
                    aicte_transaction, created = (
                        AICTEPointTransaction.objects.get_or_create(
                            student=student,
                            event=event,
                            defaults={
            ...
            log_action(
                request.user,
                f"Uploaded attendance for event {event.name}: created {created_count} attendance records, {points_created} AICTE transactions, {certificates_created} certificates",
            )
    ...
    def generate_certificates(self, request, pk=None):
        ...
        if event.status != "completed":
            raise ValidationError(
                "Certificates can only be generated for completed events."
            )
        ...
        if not path.exists(template_path) or not path.exists(metadata_path):
            raise ValidationError(
                f"Certificate template or metadata file not found: {template_type}"
            )
        ...
        if event.club and event.club.faculty_coordinator:
            ...
            if hasattr(mentor, "signature") and mentor.signature:
                faculty_signature_path = path.join(
                    settings.MEDIA_ROOT, str(mentor.signature)
                )
            elif hasattr(mentor.user, "signature") and mentor.user.signature:
                faculty_signature_path = path.join(
                    settings.MEDIA_ROOT, str(mentor.user.signature)
                )
        ...
        if active_principal_sig and active_principal_sig.signature_image:
            principal_signature_path = path.join(
                settings.MEDIA_ROOT, str(active_principal_sig.signature_image)
            )
        ...
        if existing_count > 0:
            ...
            log_action(
                request.user,
                f"Deleted {existing_count} existing certificates for event: {event.name}",
            )
        ...
        for attendance in attendees:
            ...
            try:
                ...
                certificate = Certificate.objects.create(
                    event=event, student=student, issue_date=now()
                )
                ...
                aicte_transaction = AICTEPointTransaction.objects.filter(
                    student=student, event=event
                ).first()
                code = (
                    aicte_transaction.verification_code
                    if aicte_transaction and aicte_transaction.verification_code
                    else "N/A"
                )
                ...
                certificate_url = (
                    request.build_absolute_uri(certificate.file.url)
                    if certificate.file
                    else None
                )
                try:
                    threading.Thread(
                        target=send_certificate_generation_email,
                        args=(student.user, event, certificate_url),
                    ).start()
                except Exception as e:
        ...
        log_action(
            request.user,
            f"Generated {certificate_count} certificates for event: {event.name}",
        )
...
class HallBookingViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [
        IsClubAdmin
    ]  # creation by club organizers; admin endpoints checked below
    ...
    def _has_conflict(self, hall, booking_date, start_time, end_time):
        ...
        qs = HallBooking.objects.filter(hall=hall, booking_date=booking_date).exclude(
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not all([hall_id, booking_date, start_time_str, end_time_str]):
            return Response(
                {"error": "hall, booking_date, start_time, end_time required"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {"error": "Hall not found"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        with transaction.atomic():
            ...
            if not conflict:
                booking.approved_by = (
                    request.user
                )  # automatic approval by system (user)
                booking.save()
                log_action(
                    request.user,
                    f"Auto-approved hall booking: {booking.hall.name} on {booking.booking_date} {booking.start_time}-{booking.end_time}",
                )
            else:
                log_action(
                    request.user,
                    f"Created pending hall booking: {booking.hall.name} on {booking.booking_date} {booking.start_time}-{booking.end_time}",
                )
    ...
    def approve(self, request, pk=None):
        ...
        if self._has_conflict(
            booking.hall, booking.booking_date, booking.start_time, booking.end_time
        ):
            return Response(
                {"error": "Conflicting booking found — cannot approve"},
        ...
        log_action(
            request.user,
            f"Approved hall booking: {booking.hall.name} for {booking.event.name if booking.event else 'N/A'}",
        )
        return Response(
            {"message": "Hall booking approved", "status": booking.booking_status}
        )
    ...
    def reject(self, request, pk=None):
        ...
        log_action(
            request.user,
            f"Rejected hall booking: {booking.hall.name} - Reason: {reason}",
        )
    ...
    def list_admin_pending(self, request):
        ...
        qs = HallBooking.objects.filter(booking_status="PENDING").select_related(
            "hall", "event", "booked_by"
        )
...
class CertificateTemplateViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        if self.request.user.user_type != "admin":
            raise PermissionDenied(
                "Only administrators can create certificate templates."
            )
    ...
    def perform_update(self, serializer):
        if self.request.user.user_type != "admin":
            raise PermissionDenied(
                "Only administrators can update certificate templates."
            )
...
class CertificateViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def verify(self, request, file_hash=None):
        ...
        cert = (
            Certificate.objects.filter(file_hash=file_hash)
...
class AICTECategoryViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action in ["create", "update", "partial_update", "destroy"]:
            if self.request.user.user_type != "admin":
                raise PermissionDenied(
                    "Only administrators can manage AICTE categories."
                )
        return super().get_permissions()
...
class AICTEPointTransactionViewSet(viewsets.ModelViewSet):
    ...
    def approve(self, request, pk=None):
        ...
        try:
            threading.Thread(
                target=send_points_decision_email,
                args=(tx.student.user, tx.event, tx.points_allocated, "approved"),
            ).start()
        except Exception as e:
...
class NotificationViewSet(viewsets.ModelViewSet):
    ...
    def take_action(self, request, pk=None):
        ...
        if (
            notification.notification_type == "certificate_generated"
            and notification.event
        ):
            # Open certificate download
            certificate = Certificate.objects.filter(
                event=notification.event, student__user=user
            ).first()
            if certificate:
                return Response(
                    {
                        ...
                        "file_url": request.build_absolute_uri(certificate.file.url)
                        if certificate.file
                        else None,
                    }
                )
            ...
            notification.notification_type == "points_approved"
            and notification.aicte_transaction
        ):
            ...
            notification.notification_type == "points_rejected"
            and notification.aicte_transaction
        ):
            # Open AICTE points verification page
            return Response(
            ...
            notification.notification_type == "hall_booking_approved"
            and notification.hall_booking
        ):
            ...
            notification.notification_type == "hall_booking_rejected"
            and notification.hall_booking
        ):
            # Open hall booking details
            return Response(
...
def dashboard_stats(request):
    ...
    if user.user_type == "student":
        ...
        if not mentor:
            return Response(
                {"error": "Mentor profile not found"}, status=status.HTTP_404_NOT_FOUND
            )

        mentees = Student.objects.filter(mentor=mentor)
        pending_approvals = AICTEPointTransaction.objects.filter(
            student__in=mentees, status="PENDING"
        ).count()
