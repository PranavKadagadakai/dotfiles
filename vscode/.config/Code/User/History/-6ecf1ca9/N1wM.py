...
class EventViewSet(viewsets.ModelViewSet):
    ...
    def generate_certificates(self, request, pk=None):
        """Generate certificates for event participants. For re-generation, deletes old certificates and creates new ones."""
        from os import path
        import hashlib
     ...
     file_has   certificate.save()
                ...
                Notification.objects.create(
                    ...
                    h = hashlib.sha256(pdf_buffer.read()).hexdigest()
            certificate.file_hash = file_hash
            certificate.save()

            certi
ate notifications for students
        Notib     title="Certificate Generated",
        ...
        log_xioting_count > 0n(request.user, f"Generated {certificate_count} new certificates for event: {event.name}")
msage+f" (Rplaced {existing_count} existing cetificates)"

        if er:
        response_data = {erors
          ificat cert['r']resf"{lte(rrrrr s} c  t f pnteeaf'{eedtger."
