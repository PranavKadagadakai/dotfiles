    ...
    def generate_certificates(self, request, pk=None):
        ...
        for attendance in attendees:
            ...
            try:
                ...
                # Generate QR code text with verification code if pending transaction exists
                qr_text = f"Certificate ID: {certificate.id}, Student: {student.usn}, Event: {event.name}"

                # Add verification code if there's a pending AICTE transaction for this student/event
                pending_transaction = AICTEPointTransaction.objects.filter(
                    student=student, event=event, status='PENDING'
                ).first()
                if pending_transaction and pending_transaction.verification_code:
                    qr_text += f", Verification Code: {pending_transaction.verification_code}"
...
class AICTEPointTransactionViewSet(viewsets.ModelViewSet):
    ...
    def approve(self, request, pk=None):
        ...
        if verification_code:
            ...
            if tx.verification_code != verification_code.strip():
                return Response(
