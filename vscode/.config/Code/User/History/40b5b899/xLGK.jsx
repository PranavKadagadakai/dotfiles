...
const EventAttendanceForm = ({ eventId }) => {
  ...
  const handleFileChange = async (e) => {
    ...
    // Validate the file
    const validation = await validateBulkUploadFile(f, "attendance_upload");
    ...
    if (validation.warnings.length > 0) {
      ...
      // Could show warnings to user if desired
    }
    ...
    if (name.endsWith(".csv")) {
      try {
        ...
        // Show additional validation info from our utility
        if (validation.preview && validation.preview.firstDataRow) {
          console.log("CSV Preview Info:", validation.preview);
        }
      } catch (error) {
      ...
      setAttendancePreview([
        {
          note: `XLSX file selected (${formatFileSize(
            f.size
